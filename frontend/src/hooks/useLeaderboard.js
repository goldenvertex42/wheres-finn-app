...
export function useLeaderboard() {
  ...
  const qualifiesForBoard = location.state?.qualifies;
  ...
  useEffect(() => {
    if (isEligibleSubmission && !hasSubmitted) {
      // Cleans out the browser state history buffer without triggering a page reload redirect
      navigate('/leaderboard', { replace: true, state: null });
    }
  }, [isEligibleSubmission, hasSubmitted, navigate]);
